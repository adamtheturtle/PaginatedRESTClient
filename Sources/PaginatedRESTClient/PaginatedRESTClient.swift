...
nonisolated func drivePagination<W: PagedResponse>(
    ...
    if considerParallel, let total = firstPage.total {
        estimatedPageCount = Self.pageCount(total: total, pageSize: W.pageSize)
    ...
    if considerParallel, estimatedPageCount > 1, let total = firstPage.total {
        guard let numberedTemplate else {
            throw errors.invalidRequest("Invalid pagination template")
        }
        return try await finishParallelPagination(
            W.self,
            total: total,
...
/// Completes the numbered parallel page wave and any sequential remainder.
nonisolated func finishParallelPagination<W: PagedResponse>(
    _: W.Type,
    total: Int,
